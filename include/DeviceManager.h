...
public:
  ...
  void addDevice(string type, string name) {
    Device *newDevice =
        DeviceFactory::createDevice(type, devices.size() + 1, name);
    if (newDevice) {
  ...
  Device *getDeviceByName(string namePart) {
    for (Device *d : devices) {
      if (d->getName().find(namePart) != string::npos)
        return d;
    }
    return nullptr;
  }

  vector<Device *> &getDevicesRef() { return devices; }
      ...
      if (fan) {
        if (isTurningOn) {
          ...
          if (fan->getPowerStatus()) {
            ...
            cout << "\n>> [OTOMASYON] Tuvalet isigi ACILDI -> Fan sessizlik "
                    "icin DURDURULDU."
                 << endl;
          }
        } else {
          // SENARYO 2: Işık KAPANDI -> Fan 1 DK (3 sn) Çalışsın
          cout << "\n>> [OTOMASYON] Tuvalet isigi KAPANDI -> Fan havalandirma "
                  "icin calistiriliyor..."
               << endl;
          ...
          cout << "   (Fan 1 dakika boyunca calisiyor... Lutfen bekleyin)"
               << endl;
          ...
          if (fan->getPowerStatus()) {
            ...
            cout << ">> [OTOMASYON] Sure doldu -> Fan otomatik KAPATILDI."
                 << endl;
          }
        }
      } else {
